  ...
  <div class="app-container">
    ...
    <el-dialog title="仓储出库单" :visible.sync="dialogVisible">
      <el-form :model="temp" label-position="left" label-width="70px" style="width: 100%; padding: 0 4% 0 4%;">
        ...
        <el-form-item label="出货仓库" prop="storage">
          <span>{{ temp.storage }}</span>
        </el-form-item>
        <el-form-item label="入库仓库" prop="storage">
          <el-select v-model="temp.sid2" class="filter-item" @change="handleStorageSelect">
            <el-option v-for="item in storages" :key="item.id" :label="item.label" :value="item.id" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getGroupAllStorage() {
      getGroupAllStorage({
        ...
        if (response.data.data.list && response.data.data.list.length > 0) {
          ...
          this.temp.sid2 = this.listQuery.sid
          this.getCommodityList()
        }
      })
    },
    ...
    applyData() {
      ...
      if (this.listQuery.sid === this.temp.sid2) {
        this.$message({ type: 'error', message: '出入库仓库不能相同!' })
        return
      }
      dispatch({
        id: this.userdata.user.id,
